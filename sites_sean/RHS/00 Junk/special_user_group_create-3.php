			...
			if(UserExists($userID)==false)
				...
				if($writeEnabled) 
					...
					if(NewUserInfo($userID,$password,"Blog-Gallery",$eMail,$first,$last)==false)
						{
						$error	=	1;
						$statusMessage	=	 "Cant create User - $userID";
						}
					else
...
if(!$error)
	...
	$templateFileName	=	"$gServerBaseMountPoint/messages/creo_expedition_user_welcome.html";
		...
		if($first != "" && $last != "" && $eMail != "")
			...
			if(!$error) //  && $writeEnabled
				...
				if(email_send($toAddress,$fromAddress,$fromName,$subject,$personalizedMessage)!=1)
					{
					// FIX need to send admin a mail with the error address...
					// iteratively save it up in a string and send one mail on strlen()
					print	"Error - Unable to Send Mail to $toAddress possible bad address...";
					}
				}
...
$uriString	=	"special_user_group_create-1.php?SystemUserID=$systemUserID&SystemPassword=$systemPassword&StatusMessage=$statusMessage";
